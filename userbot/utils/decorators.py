    ...
    if pattern is not None:
        if pattern.startswith(r"\#"):
            ...
            if len(Config.COMMAND_HAND_LER) == 2:
                hindureg = "^" + Config.COMMAND_HAND_LER
                reg = Config.COMMAND_HAND_LER[1]
            elif len(Config.COMMAND_HAND_LER) == 1:
                hindureg = "^\\" + Config.COMMAND_HAND_LER
                reg = Config.COMMAND_HAND_LER
            args["pattern"] = re.compile(hindureg + pattern)
    ...
    if pattern is not None:
        if pattern.startswith(r"\#"):
            ...
            if len(Config.SUDO_COMMAND_HAND_LER) == 2:
                hindureg = "^" + Config.SUDO_COMMAND_HAND_LER
...
def register(**args):
    ...
    def decorator(func):
        if not disable_edited:
            hinduub.add_event_handler(func, MessageEdited(**args))
        hinduub.add_event_handler(func, NewMessage(**args))
        try:
