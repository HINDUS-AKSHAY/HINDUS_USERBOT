...
def load_module(shortname, plugin_path=None):
    if shortname.startswith("__"):
        ...
        mod.Config = Config
        mod._format = _format
        mod.tgbot = hinduub.tgbot
        mod.sudo_cmd = sudo_cmd
        mod.CMD_HELP = CMD_HELP
        mod.reply_id = reply_id
        mod.admin_cmd = admin_cmd
        mod._hinduutils = _hinduutils
        mod._hindutools = _hindutools
...
def remove_plugin(shortname):
        ...
        for cmdname in cmd:
            if cmdname in LOADED_CMDS:
                for i in LOADED_CMDS[cmdname]:
                    hinduub.remove_event_handler(i)
    ...
    try:
        name = f"userbot.plugins.{shortname}"
        for i in reversed(range(len(hinduub._event_builders))):
            ev, cb = hinduub._event_builders[i]
            if cb.__module__ == name:
                del hinduub._event_builders[i]
    except BaseException:
        raise ValueError
