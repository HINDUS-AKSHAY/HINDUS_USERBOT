...
class Gdrive(BASE):
    __tablename__ = "hindugdrive"
    hindu = Column(String(50), primary_key=True)

    def __init__(self, hindu):
        self.hindu = hindu
...
def is_folder(folder_id):
    try:
        return SESSION.query(Gdrive).filter(Gdrive.hindu == str(folder_id))
...
def rmparent_id(folder_id):
    note = SESSION.query(Gdrive).filter(Gdrive.hindu == folder_id)
    if note:
        note.delete()
        SESSION.commit()
