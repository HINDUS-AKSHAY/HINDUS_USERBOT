...
plugin for HINDUS USERBOTUserbot
☝☝☝
...
from . import hinduub, reply_id

plugin_category = "fun"


async def mememaker(borg, msg, hindu, chat_id, reply_to_id):
...
async def hindu(event):
    ...
    msg = f"/sf {input_text}"
    hindu = await edit_or_reply(event, "```Fox is on your way...```")
    await mememaker(event.client, msg, hindu, event.chat_id, reply_to_id)
...
async def hindu(event):
    ...
    hindu = await edit_or_reply(event, "```Wait making your hardcore meme...```")
    await mememaker(event.client, msg, hindu, event.chat_id, reply_to_id)
...
async def hindu(event):
    ...
    hindu = await edit_or_reply(event, "```You can't sleep...```")
    await mememaker(event.client, msg, hindu, event.chat_id, reply_to_id)
...
async def hindu(event):
    ...
    if not input_text:
        return await edit_delete(event, "`Give me some text to process...`")
    msg = f"/sp {input_text}"
    hindu = await edit_or_reply(event, "```Yaah wait for spongebob...```")
    await mememaker(event.client, msg, hindu, event.chat_id, reply_to_id)
...
async def hindu(event):
    ...
    hindu = await edit_or_reply(event, "```Wait for your son......```")
    await mememaker(event.client, msg, hindu, event.chat_id, reply_to_id)
