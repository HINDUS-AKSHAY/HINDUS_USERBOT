...
async def stats(event):  # sourcery no-metrics
    ...
    hindu = await edit_or_reply(event, STAT_INDICATION)
...
async def stats(event):  # sourcery no-metrics
    hinducmd = event.pattern_match.group(1)
    hinduevent = await edit_or_reply(event, STAT_INDICATION)
    start_time = time.time()
...
async def _(event):
    ...
    async with event.client.conversation(chat) as conv:
        try:
            ...
            await edit_delete(hinduevent, "`unblock `@tgscanrobot` and then try`")
