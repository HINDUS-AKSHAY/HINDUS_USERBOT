...
async def promote(event):
    ...
    if not user:
        return
    hinduevent = await edit_or_reply(event, "`Promoting...`")
    try:
        await event.client(EditAdminRequest(event.chat_id, user.id, new_rights, rank))
    except BadRequestError:
        return await hinduevent.edit(NO_PERM)
    await hinduevent.edit("`Promoted Successfully! Now gib Party`")
    if BOTLOG:
        await event.client.send_message(
            BOTLOG_CHATID,
            f"#PROMOTE\
...
async def demote(event):
    ...
    if not user:
        return
    hinduevent = await edit_or_reply(event, "`Demoting...`")
    ...
    try:
        ...
        return await hinduevent.edit(NO_PERM)
    await hinduevent.edit("`Demoted Successfully! Betterluck next time`")
    if BOTLOG:
        await event.client.send_message(
            BOTLOG_CHATID,
            f"#DEMOTE\
...
async def _ban_person(event):
    ...
    hinduevent = await edit_or_reply(event, "`Whacking the pest!`")
    try:
...
async def nothanos(event):
    ...
    if not user:
        return
    hinduevent = await edit_or_reply(event, "`Unbanning...`")
    try:
        ...
        await hinduevent.edit("`Uh oh my unban logic broke!`")
    except Exception as e:
        await hinduevent.edit(f"**Error :**\n`{e}`")
...
async def endmute(event):
    ...
    if not user:
        return
    hinduevent = await edit_or_reply(event, "`Kicking...`")
    try:
        await event.client.kick_participant(event.chat_id, user.id)
    except Exception as e:
        return await hinduevent.edit(NO_PERM + f"\n{e}")
    if reason:
        await hinduevent.edit(
            f"`Kicked` [{user.first_name}](tg://user?id={user.id})`!`\nReason: {reason}"
        )
    else:
        await hinduevent.edit(f"`Kicked` [{user.first_name}](tg://user?id={user.id})`!`")
    if BOTLOG:
        await event.client.send_message(
            BOTLOG_CHATID,
            "#KICK\n"
...
async def _iundlt(event):  # sourcery no-metrics
    ...
    hinduevent = await edit_or_reply(event, "`Searching recent actions .....`")
    ...
    if not flag:
        ...
        await edit_or_reply(hinduevent, deleted_msg)
    else:
        main_msg = await edit_or_reply(hinduevent, deleted_msg)
        for msg in adminlog:
            ruser = (
                await event.client(GetFullUserRequest(msg.old.from_id.user_id))
            ).user
