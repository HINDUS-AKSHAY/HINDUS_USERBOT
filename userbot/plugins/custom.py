...
@hinduub.hindu_cmd(
    ...
    info={
        ...
        "var name": "**[List of Database Vars]**(https://HINDUS USERBOT.gitbook.io/HINDUS USERBOT/data-vars-setup)",
        "usage": [
    ...
    if vname in vlist:
        ...
        if cmd == "set":
            if not vinfo and vname == "ALIVE_TEMPLATE":
                return await edit_delete(
                    event, "**💠 Check @hindu_alive for alive teplate types.**", 60
                )
            if not vinfo:
                return await edit_delete(
...
@hinduub.hindu_cmd(
    ...
    info={
        "header": "To customize your HINDUS USERBOT.",
        "options": {
...
async def custom_HINDUS USERBOT(event):
    "To customize your HINDUS USERBOT."
    reply = await event.get_reply_message()
    text = None
    if reply:
        text = reply.text
