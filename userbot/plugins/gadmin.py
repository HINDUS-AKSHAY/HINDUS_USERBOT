...
async def hindugban(event):  # sourcery no-metrics
    "To ban user in every group where you are admin."
    hinduz = await edit_or_reply(event, "`gbanning.......`")
    start = datetime.now()
    user, reason = await get_user_from_event(event, hinduz)
    if not user:
        return
    if user.id == hinduub.uid:
        return await edit_delete(hinduz, "`why would I ban myself`")
    if str(user.id) in DEVLIST:
        return await edit_delete(hinduz, "🥴 **GBan my creator ?¿ Really‽**")
    ...
    if gban_sql.is_gbanned(user.id):
        ...
        gban_sql.hindugban(user.id, reason)
    san = await admin_groups(event.client)
    count = 0
    madboy = len(san)
    if madboy == 0:
        return await edit_delete(hinduz, "`you are not admin of atleast one group` ")
    await hinduz.edit(
    ...
    hindutaken = (end - start).seconds
    if reason:
        await hinduz.edit(
            f"[{user.first_name}](tg://user?id={user.id}) `was gbanned in {count} groups in {hindutaken} seconds`!!\n**Reason :** `{reason}`"
        )
    else:
        await hinduz.edit(
            f"[{user.first_name}](tg://user?id={user.id}) `was gbanned in {count} groups in {hindutaken} seconds`!!"
...
async def hindugban(event):
    "To unban the person from every group where you are admin."
    hinduz = await edit_or_reply(event, "`ungbanning.....`")
    start = datetime.now()
    user, reason = await get_user_from_event(event, hinduz)
    if not user:
        return
    if gban_sql.is_gbanned(user.id):
        gban_sql.hinduungban(user.id)
    else:
        return await edit_delete(
            hinduz, f"the [user](tg://user?id={user.id}) `is not in your gbanned list`"
        )
    san = await admin_groups(event.client)
    count = 0
    madboy = len(san)
    if madboy == 0:
        return await edit_delete(
            hinduz, "`you are not even admin of atleast one group `"
        )
    await hinduz.edit(
    ...
    if reason:
        await hinduz.edit(
            f"[{user.first_name}](tg://user?id={user.id}`) was ungbanned in {count} groups in {hindutaken} seconds`!!\n**Reason :** `{reason}`"
        )
    else:
        await hinduz.edit(
            f"[{user.first_name}](tg://user?id={user.id}) `was ungbanned in {count} groups in {hindutaken} seconds`!!"
...
@hinduub.hindu_cmd(incoming=True)
...
async def hindugkick(event):  # sourcery no-metrics
    "kicks the person in all groups where you are admin"
    hinduz = await edit_or_reply(event, "`gkicking.......`")
    start = datetime.now()
    user, reason = await get_user_from_event(event, hinduz)
    if not user:
        return
    if user.id == hinduub.uid:
        return await edit_delete(hinduz, "`why would I kick myself`")
    if str(user.id) in DEVLIST:
        return await edit_delete(hinduz, "**😒 I'm not going to gkick my developer!!**")
    ...
    if reason:
        await hinduz.edit(
            f"[{user.first_name}](tg://user?id={user.id}) `was gkicked in {count} groups in {hindutaken} seconds`!!\n**Reason :** `{reason}`"
        )
    else:
        await hinduz.edit(
            f"[{user.first_name}](tg://user?id={user.id}) `was gkicked in {count} groups in {hindutaken} seconds`!!"
...
async def get_full_user(event):
    ...
    if event.reply_to_msg_id and not len(args) == 2:
        ...
        try:
            ...
            return await event.edit("Error... Please report at @HINDUS_USERBOT_SUPPORT", str(err))
...
async def gben(userbot):
    ...
    try:
        ...
        pass
    if me == user:
        await hindu.edit("khud ko gpromote krega😑😑 chutiya spotted..")
        return
    try:
        if not rank:
            rank = "ㅤㅤ"
    except:
        return await hindu.edit(f"**kuch to gadbad hai😑😑**")
    if user:
        telchanel = [
            d.entity.id
        ...
        for x in telchanel:
            try:
                ...
                i += 1
                await hindu.edit(f"**Promoted in Chats **: `{i}`")
            except:
                pass
    else:
        await hindu.edit(f"**kisi ko reply kr lavde🙄!!**")
    return await hindu.edit(
...
async def gben(userbot):
    ...
    try:
        ...
        pass
    if me == user:
        await hindu.edit("khud ko gdemote karega😑😑 chumtiya spotted..")
        return
    try:
        if not rank:
            rank = "ㅤㅤ"
    except:
        return await hindu.edit(f"**Phir se kuch gadbad kiya😑**")
    if user:
        telchanel = [
            d.entity.id
            for d in await userbot.client.get_dialogs()
        ...
        for x in telchanel:
            try:
                await userbot.client(EditAdminRequest(x, user, rgt, rank))
                i += 1
                await hindu.edit(f"**Demoted in Chats **: `{i}`")
            except:
                pass
    else:
        await hindu.edit(f"**Reply to a user you dumbo !!**")
    return await hindu.edit(
...
async def gcast(event):
    ...
    tt = event.text
    msg = tt[6:]
    hindu = await edit_or_reply(event, "`Globally Broadcasting Msg...`")
    er = 0
    done = 0
    async for x in bot.iter_dialogs():
        if x.is_group:
            ...
            try:
                ...
                er += 1
    await hindu.edit(f"Done in {done} chats, error in {er} chat(s)")
