...
@hinduub.hindu_cmd(
    ...
    info={
        ...
        "examples": "{tr}msgto @HINDUS_USERBOT_SUPPORT just a testmessage",
    },
)
async def hindubroadcast_add(event):
...
async def hindubroadcast_add(event):
    ...
    hinduinput_str = event.pattern_match.group(1)
    if not hinduinput_str:
        return await edit_delete(
            event,
            "In which category should i add this chat",
            parse_mode=_format.parse_pre,
        )
    keyword = hinduinput_str.lower()
...
async def hindubroadcast_list(event):
    ...
    await edit_or_reply(hinduevent, finaloutput)
...
async def hindubroadcast_send(event):
    ...
    await edit_delete(hinduevent, resultext)
    if BOTLOG:
        await event.client.send_message(
            BOTLOG_CHATID,
...
async def hindubroadcast_remove(event):
...
async def hindubroadcast_delete(event):
    "To delete a category completely."
    hinduinput_str = event.pattern_match.group(1)
    check1 = sql.num_broadcastlist_chat(hinduinput_str)
    if check1 < 1:
        return await edit_delete(
            event,
            f"Are you sure that there is category {hinduinput_str}",
            parse_mode=_format.parse_pre,
        )
    try:
        sql.del_keyword_broadcastlist(hinduinput_str)
        await edit_or_reply(
            event,
            f"Successfully deleted the category {hinduinput_str}",
            parse_mode=_format.parse_pre,
        )
    except Exception as e:
        await edit_delete(
