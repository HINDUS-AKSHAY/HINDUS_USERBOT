...
# HINDUS USERBOT Google Drive managers  ported from Projectbish and added extra things by @HINDUS_USERBOT
...
async def download(event, gdrive, service, uri=None):  # sourcery no-metrics
    ...
    if uri:
        ...
        if hindutorrent:
            ...
            await edit_or_reply(
                gdrive,
                "`To use torrent files or download files from link install torrentutils from` @hinduplugins",
            )
...
async def upload(gdrive, service, file_path, file_name, mimeType, dir_id=None):
    ...
    body = {
        "name": file_name,
        "description": "Uploaded from Telegram using HINDUS USERBOT.",
        "mimeType": mimeType,
...
async def g_download(event):
    ...
    file_name, hinduprocess = await gdrive_download(
        event, hinduevent, service, drive_link
    )
    if hinduprocess is not None:
        return await edit_delete(hinduevent, file_name)
    ...
    if not cmd:
        ...
        await event.client.send_file(
            ...
            progress_callback=lambda d, t: asyncio.get_event_loop().create_task(
                progress(d, t, hinduevent, c_time, "Uploading...", file_name)
            ),
        )
        os.remove(file_name)
        await edit_delete(
...
async def gshare(event):
    ...
    hinduevent = await edit_or_reply(event, "`Creating sharable link...`")
    await asyncio.sleep(2)
    await share(service, hinduevent, input_str)
