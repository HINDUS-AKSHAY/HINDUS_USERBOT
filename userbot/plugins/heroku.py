# Heroku manager for your HINDUS USERBOT
...
async def variable(var):  # sourcery no-metrics
    ...
    if exe == "get":
        hindu = await edit_or_reply(var, "`Getting information...`")
        ...
        hindu = await edit_or_reply(var, "`Setting information...`")
        if not variable:
            return await hindu.edit("`.set var <ConfigVars-name> <value>`")
        ...
        if variable in heroku_var:
            await hindu.edit(f"`{variable}` **successfully changed to  ->  **`{value}`")
        else:
            await hindu.edit(
        ...
        hindu = await edit_or_reply(var, "`Getting information to deleting variable...`")
        try:
            variable = var.pattern_match.group(2).split()[0]
        except IndexError:
            return await hindu.edit("`Please specify ConfigVars you want to delete`")
        await asyncio.sleep(1.5)
        if variable not in heroku_var:
            return await hindu.edit(f"`{variable}`**  does not exist**")

        await hindu.edit(f"`{variable}`  **successfully deleted**")
