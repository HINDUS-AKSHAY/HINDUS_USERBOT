...
from ..helpers.utils import _hinduutils
...
async def spam_function(event, nadan, hindu, sleeptimem, sleeptimet, DelaySpam=False):
    # sourcery no-metrics
    counter = int(hindu[0])
    if len(hindu) == 2:
        spam_message = str(hindu[1])
...
async def spammer(event):
    ...
    await spam_function(event, nadan, hindu, sleeptimem, sleeptimet)
...
@hinduub.hindu_cmd(
    ...
    info={
        ...
        "usage": "{tr}cspam <text>",
        "examples": "{tr}cspam HINDUS USERBOT",
    },
)
...
@hinduub.hindu_cmd(
    ...
    info={
        ...
        "usage": "{tr}wspam <text>",
        "examples": "{tr}wspam I am using HINDUS USERBOT",
    },
)
async def tmeme(event):
...
async def spammer(event):
    ...
    await event.delete()
    addgvar("spamwork", True)
    await spam_function(event, reply, hindu, sleeptimem, sleeptimet, DelaySpam=True)
