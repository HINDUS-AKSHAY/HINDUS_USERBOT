...
@hinduub.hindu_cmd(
    ...
    info={
        ...
        "usage": "{tr}hash <text>",
        "examples": "{tr}hash HINDUS USERBOT",
    },
)
...
@hinduub.hindu_cmd(
    ...
    info={
        ...
        "examples": ["{tr}hbase en HINDUS USERBOT", "{tr}hbase de Q2F0dXNlcmJvdA=="],
    },
)
    ...
    if event.pattern_match.group(1) == "en":
        if string:
            ...
            if mediatype is None:
                ...
                hinduevent = await edit_or_reply(event, "`Encoding ...`")
                c_time = time.time()
        ...
        await edit_or_reply(
            hinduevent, result, file_name="encodedfile.txt", caption="It's Encoded"
        )
    else:
        try:
            lething = str(
