...
async def _(event):
    ...
    user, rank = await get_user_from_event(event, hinduevent)
    if not rank:
        rank = "Admin"
    if not user:
        return
    try:
        ...
        return await hinduevent.edit("__I think you don't have permission to promote__")
    except Exception as e:
        return await edit_delete(hinduevent, f"__{e}__", time=10)
