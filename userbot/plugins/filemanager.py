...
from ..helpers.utils import _format, _hinduutils
...
async def lst(event):  # sourcery no-metrics
    ...
    hindu = "".join(event.text.split(maxsplit=1)[1:])
    path = hindu or os.getcwd()
    if not os.path.exists(path):
        await edit_or_reply(
            event,
            f"there is no such directory or file with the name `{hindu}` check again",
        )
        return
    path = Path(hindu) if hindu else os.getcwd()
    if os.path.isdir(path):
        ...
        for contents in sorted(lists):
            hindupath = os.path.join(path, contents)
            if not os.path.isdir(hindupath):
                size = os.stat(hindupath).st_size
...
async def lst(event):
    ...
    hindu = event.pattern_match.group(1)
    if hindu:
        path = Path(hindu)
    else:
    ...
    if not os.path.exists(path):
        ...
        return
    hinducmd = f"rm -rf {path}"
    if os.path.isdir(path):
        await _hinduutils.runcmd(hinducmd)
...
async def _(event):
    ...
    try:
        await _hinduutils.runcmd(f"mkdir {original}")
...
async def _(event):
    ...
    await asyncio.sleep(2)
    try:
        await _hinduutils.runcmd(f"cp -r {original} {location}")
