...
from . import _hindutools, convert_toimage, deEmojify, hinduub
...
async def ocr(event):
    ...
    hinduevent = await edit_or_reply(event, "`Reading...`")
    ...
    try:
        ...
        await hinduevent.edit("`Couldn't read it.. you sure this readable !?`")
    test_file = await ocr_space_file(filename=output_file, language=lang_code)
    try:
        ParsedText = test_file["ParsedResults"][0]["ParsedText"]
    except BaseException:
        await hinduevent.edit("`Couldn't read it.`\n`I guess I need new glasses.`")
    else:
        if cmd == "":
