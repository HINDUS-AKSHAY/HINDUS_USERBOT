...
from ..helpers.utils import _format, _hinduutils, reply_id
...
async def currency(event):
    ...
    try:
        ...
        symbols = await AioHttp().get_raw(
            "https://raw.githubusercontent.com/OFFICIAL-SAMEER/HINDUS USERBOT-Resources/master/Resources/Data/currency.py"
        )
...
async def _(event):
    ...
    chat = "@VS_Robot"
    hinduevent = await edit_or_reply(event, " `Sliding my tip, of fingers over it`")
    async with event.client.conversation(chat) as conv:
        ...
        if not input_str:
            return await edit_or_reply(hinduevent, response4.text)
...
async def parseqr(event):
    ...
    hinduevent = await edit_or_reply(event, "`Decoding....`")
    ...
    t_response, e_response = (await _hinduutils.runcmd(command_to_exec))[:2]
    if os.path.exists(downloaded_file_name):
        os.remove(downloaded_file_name)
    soup = BeautifulSoup(t_response, "html.parser")
    try:
        qr_contents = soup.find_all("pre")[0].text
        await edit_or_reply(hinduevent, f"**The decoded message is :**\n`{qr_contents}`")
    except IndexError:
        result = soup.text
        await edit_or_reply(hinduevent, f"**Failed to Decode:**\n`{result}`")
    except Exception as e:
        await edit_or_reply(hinduevent, f"**Error:**\n`{e}`")
...
async def _(event):
    ...
    hinduevent = await edit_or_reply(event, "...")
    start = datetime.now()
    input_str = event.pattern_match.group(1)
    ...
    try:
        ...
        return await hinduevent.edit(str(e))
    end = datetime.now()
    ms = (end - start).seconds
    await edit_delete(hinduevent, "Created BarCode in {} seconds".format(ms))
...
async def _(event):
    ...
    hinduevent = await edit_or_reply(event, "`processiong...........`")
    input_str = event.pattern_match.group(1)
    xkcd_id = None
    if input_str:
    ...
    if not r.ok:
        return await hinduevent.edit("xkcd n.{} not found!".format(xkcd_id))
    data = r.json()
    year = data.get("year")
...
Year: {}""".format(
    ...
    await hinduevent.edit(output_str, link_preview=True)
