...
from ..helpers.utils import _hindutools
from ..sql_helper.globals import gvarstatus

plugin_category = "fun"

# modified and developed by @HINDUS_USERBOT
...
def verify_cond(hinduarray, text):
    return any(i in text for i in hinduarray)


def pack_name(userid, pack, is_anim):
    if is_anim:
        return f"HINDUS USERBOT_{userid}_{pack}_anim"
    return f"HINDUS USERBOT_{userid}_{pack}"
...
async def newpacksticker(
    ...
    try:
        ...
        await hinduevent.edit("You have blocked the @stickers bot. unblock it and try.")
        if not pkang:
...
async def add_to_pack(
    ...
    while ("50" in x.text) or ("120" in x.text):
        ...
        await hinduevent.edit(f"`Switching to Pack {pack} due to insufficient space`")
        await conv.send_message(packname)
        x = await conv.get_response()
...
async def kang(args):  # sourcery no-metrics
    ...
    if not user.username:
        try:
            ...
            username = f"hindu_{user.id}"
    else:
        username = user.username
    userid = user.id
    if message and message.media:
        if isinstance(message.media, MessageMediaPhoto):
            hinduevent = await edit_or_reply(args, f"`{random.choice(KANGING_STR)}`")
            photo = io.BytesIO()
    ...
    if photo:
        ...
        if len(splat) == 2:
            if char_is_emoji(splat[0][0]):
                if char_is_emoji(splat[1][0]):
                    return await hinduevent.edit("check `.info stickers`")
        ...
        if (
            ...
            await hinduevent.edit("`Brewing a new Pack...`")
            async with args.client.conversation("@Stickers") as conv:
...
async def pack_kang(event):  # sourcery no-metrics
    ...
    hindu = base64.b64decode("QUFBQUFGRV9vWjVYVE5fUnVaaEtOdw==")
    ...
    for message in reqd_sticker_set.documents:
        if "image" in message.mime_type.split("/"):
            ...
            await edit_delete(hinduevent, "`Unsupported File!`")
            return
        if photo:
            ...
            if (
                ...
                async with event.client.conversation("@Stickers") as conv:
                    pack, HINDUS USERBOTname = await newpacksticker(
                        hinduevent,
                        conv,
                        cmd,
                        event,
                ...
                async with event.client.conversation("@Stickers") as conv:
                    pack, HINDUS USERBOTname = await add_to_pack(
                        hinduevent,
                        conv,
                        event,
                        packname,
            ...
            if HINDUS USERBOTname is None:
                return
            if HINDUS USERBOTname not in blablapacks:
                blablapacks.append(HINDUS USERBOTname)
...
async def pic2packcmd(event):
    ...
    hinduevent = await edit_or_reply(event, "__🔪Cropping and adjusting the image...__")
    try:
        ...
        emoji = "▫️"
    chat = "@Stickers"
    name = "HINDUS USERBOT_" + "".join(
        random.choice(list(string.ascii_lowercase + string.ascii_uppercase))
        for _ in range(16)
    )
    image = await _hindutools.media_to_pic(hinduevent, reply, noedits=True)
    if image[1] is None:
        return await edit_delete(
    ...
    newimg.save(new_img)
    new_img.seek(0)
    hinduevent = await event.edit("__Making the pack.__")
...
async def get_pack_info(event):
    ...
    if not isinstance(stickerset_attr, DocumentAttributeSticker):
        return await hinduevent.edit("`This is not a sticker. Reply to a sticker.`")
    get_stickerset = await event.client(
        GetStickerSetRequest(
            InputStickerSetID(
                id=stickerset_attr.stickerset.id,
...
async def cb_sticker(event):
    ...
    hinduevent = await edit_or_reply(event, "`Searching sticker packs....`")
    scraper = cloudscraper.create_scraper()
    text = scraper.get(combot_stickers_url + split).text
    soup = bs(text, "lxml")
    results = soup.find_all("div", {"class": "sticker-pack__header"})
    if not results:
        return await edit_delete(hinduevent, "`No results found :(.`", 5)
