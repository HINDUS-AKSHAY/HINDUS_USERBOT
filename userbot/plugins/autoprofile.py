...
async def autopicloop():
    ...
    if AUTOPICSTART and Config.DEFAULT_PIC is None:
        if BOTLOG:
            return await hinduub.send_message(
                BOTLOG_CHATID,
    ...
    while AUTOPICSTART:
        ...
        file = await hinduub.upload_file(autophoto_path)
        try:
            await hinduub(functions.photos.UploadProfilePhotoRequest(file))
...
async def custompfploop():
    ...
    while CUSTOMPICSTART:
        ...
        file = await hinduub.upload_file("donottouch.jpg")
        try:
            if i > 0:
                await hinduub(
                    functions.photos.DeletePhotosRequest(
                        await hinduub.get_profile_photos("me", limit=1)
                    )
                )
            i += 1
            await hinduub(functions.photos.UploadProfilePhotoRequest(file))
...
async def digitalpicloop():
    ...
    while DIGITALPICSTART:
        ...
        fnt = ImageFont.truetype(hindu, 200)
...
async def autoname_loop():
    ...
    while AUTONAMESTART:
        ...
        LOGS.info(name)
        try:
            await hinduub(functions.account.UpdateProfileRequest(first_name=name))
...
async def autobio_loop():
    ...
    while AUTOBIOSTART:
        ...
        LOGS.info(bio)
        try:
            await hinduub(functions.account.UpdateProfileRequest(about=bio))
        except FloodWaitError as ex:
            LOGS.warning(str(ex))
            await asyncio.sleep(ex.seconds)
        await asyncio.sleep(Config.CHANGE_TIME)
...
async def autopfp_start():
    ...
    while AUTOPFP_START:
        ...
        i += 1
        await hinduub(functions.photos.UploadProfilePhotoRequest(file))
        await _hinduutils.runcmd("rm -rf donottouch.jpg")
        await asyncio.sleep(Config.CHANGE_TIME)
...
hinduub.loop.create_task(autopfp_start())
hinduub.loop.create_task(autopicloop())
hinduub.loop.create_task(digitalpicloop())
hinduub.loop.create_task(bloom_pfploop())
hinduub.loop.create_task(autoname_loop())
hinduub.loop.create_task(autobio_loop())
hinduub.loop.create_task(custompfploop())
