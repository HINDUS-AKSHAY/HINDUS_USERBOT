...
# baning spmmers plugin for HINDUS USERBOT by @HINDUS_USERBOT
...
from . import BOTLOG, BOTLOG_CHATID, edit_or_reply, hinduub, logging, spamwatch

LOGS = logging.getLogger(__name__)
plugin_category = "admin"
if Config.ANTISPAMBOT_BAN:

    @hinduub.on(ChatAction())
    async def anti_spambot(event):  # sourcery no-metrics
        if not event.user_joined and not event.user_added:
            return
        user = await event.get_user()
        hinduadmin = await is_admin(event.client, event.chat_id, event.client.uid)
        if not hinduadmin:
            return
        hindubanned = None
        adder = None
        ...
        if is_gbanned(user.id):
            hindugban = get_gbanuser(user.id)
            if hindugban.reason:
                hmm = await event.reply(
                    f"[{user.first_name}](tg://user?id={user.id}) was gbanned by you for the reason `{hindugban.reason}`"
                )
            else:
                hmm = await event.reply(
            ...
            try:
                ...
                LOGS.info(e)
        if spamwatch and not hindubanned:
...
async def caschecker(event):
    ...
    try:
        ...
        await hinduevent.edit("`CAS check failed: Admin privileges are required`")
        return
    except BaseException:
        await hinduevent.edit("`CAS check failed`")
        return
    await hinduevent.edit(text)
...
async def caschecker(event):
    ...
    try:
        ...
        await hinduevent.edit("`spamwatch check failed: Admin privileges are required`")
        return
    except BaseException:
        await hinduevent.edit("`spamwatch check failed`")
        return
    await hinduevent.edit(text)
