# telegraph utils for HINDUS USERBOT
import os
import random
import string
...
async def _(event):
    ...
    if input_str in ["media", "m"]:
        ...
        await hinduevent.edit(f"`Downloaded to {downloaded_file_name}`")
        ...
        ms = (end - start).seconds
        hindu = f"https://telegra.ph/{response['path']}"
        await hinduevent.edit(
            f"**➥ Uploaded to :-** [telegraph]({hindu})\
                 \n**➥ Uploaded in {ms} seconds.**\
                 \n**➥ Uploaded by :-** {mention}",
            link_preview=True,
        )
