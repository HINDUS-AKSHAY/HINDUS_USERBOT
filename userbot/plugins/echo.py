...
async def echo(event):
    ...
    hinduevent = await edit_or_reply(event, "`Adding Echo to user...`")
    user, rank = await get_user_from_event(event, hinduevent, nogroup=True)
    if not user:
        return
    ...
    try:
        ...
        await edit_or_reply(hinduevent, "Hi")
...
@hinduub.hindu_cmd(incoming=True, edited=False)
