# by  @HINDUS_USERBOT ( https://t.me/HINDUS_USERBOT  )

# songs finder for HINDUS USERBOT
import asyncio
import base64
import io
import os
...
from ..helpers.utils import _hinduutils, reply_id
...
async def _(event):
    ...
    hinduevent = await edit_or_reply(event, "`wi8..! I am finding your song....`")
    ...
    try:
        ...
        pass
    stderr = (await _hinduutils.runcmd(song_cmd))[1]
    if stderr:
        return await hinduevent.edit(f"**Error :** `{stderr}`")
    hinduname, stderr = (await _hinduutils.runcmd(name_cmd))[:2]
    if stderr:
        return await hinduevent.edit(f"**Error :** `{stderr}`")
    # stderr = (await runcmd(thumb_cmd))[1]
    hinduname = os.path.splitext(hinduname)[0]
    # if stderr:
    #    return await hinduevent.edit(f"**Error :** `{stderr}`")
    song_file = Path(f"{hinduname}.mp3")
    if not os.path.exists(song_file):
        return await hinduevent.edit(
            f"Sorry!. I can't find any related video/audio for `{query}`"
        )
    await hinduevent.edit("`yeah..! i found something wi8..🥰`")
    hinduthumb = Path(f"{hinduname}.jpg")
    if not os.path.exists(hinduthumb):
        hinduthumb = Path(f"{hinduname}.webp")
    elif not os.path.exists(hinduthumb):
        hinduthumb = None
    ...
    for files in (hinduthumb, song_file):
...
async def _(event):
    ...
    stderr = (await _hinduutils.runcmd(video_cmd))[1]
    if stderr:
        return await hinduevent.edit(f"**Error :** `{stderr}`")
    hinduname, stderr = (await _hinduutils.runcmd(name_cmd))[:2]
    if stderr:
    ...
    #    return await hinduevent.edit(f"**Error :** `{stderr}`")
    hinduname = os.path.splitext(hinduname)[0]
    vsong_file = Path(f"{hinduname}.mp4")
    if not os.path.exists(vsong_file):
        vsong_file = Path(f"{hinduname}.mkv")
    ...
    await hinduevent.delete()
    for files in (hinduthumb, vsong_file):
        if files and os.path.exists(files):
            os.remove(files)
...
async def shazamcmd(event):
    ...
    hinduevent = await edit_or_reply(event, "__Downloading the audio clip...__")
    try:
        ...
        LOGS.error(e)
        return await edit_delete(
            hinduevent, f"**Error while reverse searching song:**\n__{e}__"
        )
...
async def _(event):
    ...
    hinduevent = await edit_or_reply(event, SONG_SEARCH_STRING, parse_mode="html")
    async with event.client.conversation(chat) as conv:
        try:
            ...
            if baka[0].message.startswith(
                ...
                return await edit_delete(
                    hinduevent, SONG_NOT_FOUND, parse_mode="html", time=5
                )
            await hinduevent.edit(SONG_SENDING_STRING, parse_mode="html")
            ...
            return await hinduevent.edit(SONGBOT_BLOCKED_STRING, parse_mode="html")
        await event.client.send_file(
            event.chat_id,
            music,
...
async def _(event):
    ...
    chat = "@auddbot"
    hinduevent = await edit_or_reply(event, "```Identifying the song```")
    async with event.client.conversation(chat) as conv:
        try:
            ...
            await hinduevent.edit("Wait just a sec...")
            result = await conv.get_response()
            await event.client.send_read_acknowledge(conv.chat_id)
        except YouBlockedUserError:
            await hinduevent.edit("```Please unblock (@auddbot) and try again```")
            return
    namem = f"**Song Name : **`{result.text.splitlines()[0]}`\
        \n\n**Details : **__{result.text.splitlines()[2]}__"
    await hinduevent.edit(namem)
