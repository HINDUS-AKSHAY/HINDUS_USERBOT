...
async def group_has_sedbot(group):
    if isinstance(group, types.InputPeerChannel):
        full = await hinduub(functions.channels.GetFullChannelRequest(group))
    elif isinstance(group, types.InputPeerChat):
        full = await hinduub(functions.messages.GetFullChatRequest(group.chat_id))
    else:
        return False
...
@hinduub.on(events.NewMessage)
async def on_message(event):
    last_msgs[event.chat_id].appendleft(event.message)


@hinduub.on(events.MessageEdited)
