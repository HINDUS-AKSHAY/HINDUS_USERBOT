# speech to text module for HINDUS USERBOT by uniborg (@spechide)
import os
...
async def _(event):
    ...
    hinduevent = await edit_or_reply(event, "`Downloading to my local, for analysis  🙇`")
    required_file_name = await event.client.download_media(reply, Config.TEMP_DIR)
    await hinduevent.edit("`Starting analysis, using IBM WatSon Speech To Text`")
    headers = {
    ...
    r = response.json()
    if "results" not in r:
        return await hinduevent.edit(r["error"])
    ...
    await hinduevent.edit(string_to_show)
