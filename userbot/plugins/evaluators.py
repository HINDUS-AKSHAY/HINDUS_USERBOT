...
@hinduub.hindu_cmd(
    ...
    info={
        ...
        "examples": "{tr}exec hindu stringsetup.py",
    },
)
async def _(event):
    ...
    hinduevent = await edit_or_reply(event, "`Executing.....`")
    ...
    curruser = Lionuser.username or "HINDUS USERBOT"
    uid = os.geteuid()
    if uid == 0:
...
@hinduub.hindu_cmd(
    ...
    info={
        ...
        "examples": "{tr}eval print('HINDUS USERBOT')",
    },
)
async def _(event):
    ...
    hinduevent = await edit_or_reply(event, "`Running ...`")
