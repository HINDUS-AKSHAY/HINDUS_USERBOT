# reverse search and google search  plugin for hindu
import io
import os
import re
import urllib
...
@hinduub.hindu_cmd(
    ...
    info={
        ...
        "examples": [
            "{tr}gs HINDUS USERBOT",
            "{tr}gs -l6 HINDUS USERBOT",
            "{tr}gs -p2 HINDUS USERBOT",
            "{tr}gs -p2 -l7 HINDUS USERBOT",
        ],
    },
)
async def gsearch(q_event):
    "Google search command."
    hinduevent = await edit_or_reply(q_event, "`searching........`")
...
async def _(event):
    ...
    if event.reply_to_msg_id:
        hinduevent = await edit_or_reply(event, "Pre Processing Media")
        ...
        if previous_message.media:
            ...
            the_location = google_rs_response.headers.get("Location")
        await hinduevent.edit("Found Google Result. Pouring some soup on it!")
        headers = {
        ...
        try:
            ...
            return await edit_delete(
                hinduevent, "`Sorry. I am unable to find similar images`"
            )
        end = datetime.now()
...
<i>fetched in {ms} seconds</i>""".format(
        ...
        hinduevent = event
    await edit_or_reply(hinduevent, OUTPUT_STR, parse_mode="HTML", link_preview=False)
...
async def _(img):
    ...
    if message and message.media:
        ...
        return
    if photo:
        hinduevent = await edit_or_reply(img, "`Processing...`")
        try:
            image = Image.open(photo)
        except OSError:
            return await hinduevent.edit("`Unsupported , most likely.`")
        name = "okgoogle.png"
        image.save(name, "PNG")
        image.close()
        ...
        if response != 400:
            ...
            return await hinduevent.edit("`Unable to perform reverse search.`")
        ...
        if guess and imgspage:
            ...
            return await hinduevent.edit("`Can't find any kind similar images.`")
