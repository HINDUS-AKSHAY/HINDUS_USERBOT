# Made by @HINDUS_USERBOT and @HINDUS_USERBOT
# memify plugin for HINDUS USERBOT
import asyncio
import base64
import io
import os
...
from ..helpers import asciiart, hindu_meeme, hindu_meme, media_type
...
async def maccmd(event):  # sourcery no-metrics
    ...
    hinduevent = await event.edit("__Adding frame for media....__")
    args = event.pattern_match.group(1)
    force = bool(args)
    try:
        imag = await _hindutools.media_to_pic(hinduevent, reply, noedits=True)
        if imag[1] is None:
            return await edit_delete(
        ...
        return await edit_delete(hinduevent, f"**Error in identifying image:**\n__{e}__")
    wid, hgt = image.size
...
async def memes(event):
    ...
    meme = os.path.join("./temp", "hindumeme.jpg")
    if gvarstatus("CNG_FONTS") is None:
        CNG_FONTS = "userbot/helpers/styles/Vampires.ttf"
    else:
        CNG_FONTS = gvarstatus("CNG_FONTS")
    if max(len(top), len(bottom)) < 21:
        await hindu_meme(CNG_FONTS, top, bottom, meme_file, meme)
    else:
        await hindu_meeme(top, bottom, CNG_FONTS, meme_file, meme)
    if cmd != "mmf":
        meme = convert_tosticker(meme)
    await event.client.send_file(
        event.chat_id, meme, reply_to=hinduid, force_document=False
    )
...
async def lang(event):
    ...
    if input_str not in font_list:
        ...
        await hinduevent.edit(f"**Available Fonts names are here:-**\n\n{FONTS}")
    else:
...
async def memes(event):
    ...
    color1 = c_list[0]
    color2 = c_list[1]
    bgcolor = "#080808" if not hinduinput else hinduinput
    asciiart(meme_file, 0.3, 1.9, outputfile, color1, color2, bgcolor)
    await event.client.send_file(
        event.chat_id, outputfile, reply_to=hinduid, force_document=False
    )
    await output[0].delete()
...
async def memes(event):
    ...
    await event.client.send_file(
        event.chat_id, outputfile, force_document=False, reply_to=hinduid
    )
    await output[0].delete()
    for files in (outputfile, meme_file):
        if files and os.path.exists(files):
...
async def memes(event):
    ...
    hinduinput = event.pattern_match.group(1)
    hinduinput = 50 if not hinduinput else int(hinduinput)
    reply = await event.get_reply_message()
    if not reply:
        return await edit_delete(event, "`Reply to supported Media...`")
    hmm = base64.b64decode("QUFBQUFGRV9vWjVYVE5fUnVaaEtOdw==")
    ...
    try:
        await crop(meme_file, outputfile, hinduinput)
    except Exception as e:
...
async def memes(event):
    ...
    if not hinduinput:
        hinduinput = "50"
    if ";" in str(hinduinput):
        hinduinput, colr = hinduinput.split(";", 1)
    else:
        colr = 0
    ...
    try:
        await add_frame(meme_file, outputfile, hinduinput, colr)
    except Exception as e:
        return await output[0].edit(f"`{e}`")
    try:
        await event.client.send_file(
