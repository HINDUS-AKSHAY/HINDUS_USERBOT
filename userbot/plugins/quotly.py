...
@hinduub.hindu_cmd(
    ...
    info={
        ...
        "examples": ["{tr}qpic HINDUS USERBOT.", "{tr}qpic -b HINDUS USERBOT."],
    },
)
async def q_pic(event):  # sourcery no-metrics
    ...
    hinduevent = await edit_or_reply(event, "__Making Quote pic....__")
    ...
    output = io.BytesIO()
    if sticker:
        output.name = "HINDUS USERBOT.Webp"
        img.save(output, "webp")
    else:
        output.name = "HINDUS USERBOT.png"
        img.save(output, "PNG")
    output.seek(0)
...
async def stickerchat(hinduquotes):
    "Makes your message as sticker quote"
    reply = await hinduquotes.get_reply_message()
    if not reply:
        return await edit_or_reply(
            hinduquotes, "`I cant quote the message . reply to a message`"
        )
    fetchmsg = reply.message
    repliedreply = None
    mediatype = media_type(reply)
    if mediatype and mediatype in ["Photo", "Round Video", "Gif"]:
        return await edit_or_reply(hinduquotes, "`Replied message is not supported now`")
    hinduevent = await edit_or_reply(hinduquotes, "`Making quote...`")
    user = (
        await hinduquotes.client.get_entity(reply.forward.sender)
        if reply.fwd_from
        else reply.sender
    )
    res, hindumsg = await process(fetchmsg, user, hinduquotes.client, reply, repliedreply)
    if not res:
        return
    outfi = os.path.join("./temp", "sticker.png")
    hindumsg.save(outfi)
    endfi = convert_tosticker(outfi)
    await hinduquotes.client.send_file(hinduquotes.chat_id, endfi, reply_to=reply)
    await hinduevent.delete()
    os.remove(endfi)
...
async def _(event):
    ...
    chat = "@QuotLyBot"
    hinduevent = await edit_or_reply(event, "```Making a Quote```")
    async with event.client.conversation(chat) as conv:
        try:
            ...
            if messages_id != []:
                ...
                return await edit_delete(
                    hinduevent, "`I guess you have used a invalid syntax`"
                )
            response = await response
        except YouBlockedUserError:
            return await hinduevent.edit("```Please unblock me (@QuotLyBot) u Nigga```")
