...
from . import edit_or_reply, hinduub, reply_id
...
async def _(event):
    ...
    hinduevent = await edit_or_reply(event, "```Gathering info...```")
    async with event.client.conversation(chat) as conv:
        try:
            ...
            await hinduevent.edit("Unblock @cricbuzz_bot & try again")
            return
        if respond.text.startswith("I can't find that"):
            await hinduevent.edit("sorry i can't find it")
        else:
