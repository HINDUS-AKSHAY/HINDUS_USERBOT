...
# HINDUS USERBOT module for having some fun with people.
import asyncio
import random
import re
...
from ..helpers import hindumemes
from ..helpers.utils import _hinduutils, parse_pre
...
@hinduub.hindu_cmd(
    ...
    info={
        ...
        "examples": [
            "{tr}squirrelsay HINDUS USERBOT",
            "{tr}milksay HINDUS USERBOT",
            "{tr}ghostbustersghostbusterssay HINDUS USERBOT",
        ],
    },
)
...
async def who(event):
    ...
    if replied_user is None:
        return
    caption = await hindumemes.slap(replied_user, event, mention)
    try:
...
async def faces(owo):
    ...
    reply_text = re.sub(r"\!+", " " + random.choice(hindumemes.UWUS), reply_text)
    reply_text = reply_text.replace("ove", "uv")
    reply_text += " " + random.choice(hindumemes.UWUS)
...
async def gbun(event):
    ...
    hinduevent = await edit_or_reply(event, "**Summoning out le Gungnir ❗️⚜️☠️**")
    ...
    if event.reply_to_msg_id:
        ...
        if idd == 1035034432:
            ...
            if len(gbunVar) > 0:
                ...
                jnl += no_reason
            await hinduevent.edit(jnl)
    else:
        mention = "`Warning!! User 𝙂𝘽𝘼𝙉𝙉𝙀𝘿 By Admin...\nReason: Potential spammer. `"
        await hinduevent.edit(mention)
