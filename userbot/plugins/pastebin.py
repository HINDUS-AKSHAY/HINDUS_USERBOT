...
async def paste_img(event):
    ...
    d_file_name = None
    hinduevent = await edit_or_reply(event, "`Pasting the text on image`")
    ...
    try:
        ...
        await edit_delete(hinduevent, f"**Error:**\n`{e}`", time=10)
...
async def paste_bin(event):
    ...
    hinduevent = await edit_or_reply(event, "`pasting text to paste bin....`")
    ...
    try:
        ...
        await hinduevent.edit(result, link_preview=False, parse_mode="html")
    except Exception as e:
        await edit_delete(hinduevent, f"**Error while pasting text:**\n`{e}`")
...
async def get_dogbin_content(event):
    ...
    hinduevent = await edit_or_reply(event, "`Getting Contents of pastebin.....`")
    rawurl = None
    if "raw" in url:
        rawurl = url
    if rawurl is None:
        ...
        elif "hindubin" in url:
            rawurl = f"http://hindubin.up.railway.app/raw/{fid[0]}"
        else:
    ...
    try:
        ...
        return await hinduevent.edit(f"**Request timed out.**__{str(TimeoutErr)}__")
    ...
    await edit_or_reply(hinduevent, reply_text)
...
async def _(event):
    ...
    try:
        ...
        return await edit_delete(hinduevent, f"**Error while pasting text:**\n`{e}`")
    url = response["url"]
    chat = "@CorsaBot"
    await hinduevent.edit("`Making instant view...`")
    async with event.client.conversation(chat) as conv:
        try:
            ...
            return await hinduevent.edit("```Please unblock me (@CorsaBot) and try```")
        result = ""
        if response:
        ...
        await hinduevent.edit(result, link_preview=True)
