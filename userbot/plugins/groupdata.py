...
@hinduub.hindu_cmd(
    ...
    info={
        ...
        "examples": "{tr}admins @HINDUS_USERBOT_SUPPORT",
    },
)
async def _(event):
...
@hinduub.hindu_cmd(
    ...
    info={
        ...
        "examples": "{tr}bots @HINDUS_USERBOT_SUPPORT",
    },
)
async def _(event):
    "To get list of bots."
...
async def get_users(show):
    ...
    hinduevent = await edit_or_reply(show, "`getting users list wait...`  ")
    try:
        ...
        mentions += " " + str(e) + "\n"
    await edit_or_reply(hinduevent, mentions)
...
@hinduub.hindu_cmd(
    ...
    info={
        ...
        "examples": "{tr}chatinfo @HINDUS_USERBOT_SUPPORT",
    },
)
async def info(event):
    "To get group information"
    hinduevent = await edit_or_reply(event, "`Analysing the chat...`")
    chat = await get_chatinfo(event, hinduevent)
    if chat is None:
        return
    caption = await fetch_info(chat, event)
    try:
        await hinduevent.edit(caption, parse_mode="html")
    except Exception as e:
        if BOTLOG:
        ...
        await hinduevent.edit("`An unexpected error has occurred.`")


async def get_chatinfo(event, hinduevent):
    ...
    try:
        ...
        try:
            ...
            await hinduevent.edit("`Invalid channel/group`")
            return None
            ...
            await hinduevent.edit("`Channel or supergroup doesn't exist`")
            return None
        except (TypeError, ValueError) as err:
            LOGS.info(err)
            await edit_delete(hinduevent, "**Error:**\n__Can't fetch the chat__")
            return None
    return chat_info
