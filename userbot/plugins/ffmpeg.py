...
from ..helpers import _hindutools, media_type, progress, reply_id

plugin_category = "utils"


FF_MPEG_DOWN_LOAD_MEDIA_PATH = os.path.join(
    Config.TMP_DOWNLOAD_DIRECTORY, "HINDUS USERBOT.media.ffmpeg"
)
...
async def ff_mpeg_trim_cmd(event):
    ...
    if not os.path.exists(FF_MPEG_DOWN_LOAD_MEDIA_PATH):
        ...
        if reply_message:
            ...
            hinduevent = await edit_or_reply(event, "`Saving the file...`")
            try:
                c_time = time.time()
                ...
                await event.client.fast_download_file(
                    ...
                    progress_callback=lambda d, t: asyncio.get_event_loop().create_task(
                        progress(d, t, hinduevent, c_time, "trying to download")
                    ),
                )
                dl.close()
...
async def ff_mpeg_trim_cmd(event):
    ...
    hinduevent = await edit_or_reply(event, "`Triming the media......`")
    ...
    if len(cmt) == 3:
        ...
        if o is None:
            return await edit_delete(
                hinduevent, f"**Error : **`Can't complete the process`"
            )
        try:
            c_time = time.time()
            await event.client.send_file(
                ...
                progress_callback=lambda d, t: asyncio.get_event_loop().create_task(
                    progress(d, t, hinduevent, c_time, "trying to upload")
                ),
            )
            os.remove(o)
        except Exception as e:
            return await edit_delete(hinduevent, f"**Error : **`{e}`")
    elif len(cmt) == 2:
        # output should be image
        cmd, start_time = cmt
        o = await _hindutools.take_screen_shot(FF_MPEG_DOWN_LOAD_MEDIA_PATH, start_time)
        if o is None:
            return await edit_delete(
                hinduevent, "**Error : **`Can't complete the process`"
            )
        try:
            c_time = time.time()
            await event.client.send_file(
        ...
        await edit_delete(hinduevent, "RTFM")
        return
    end = datetime.now()
    ms = (end - start).seconds
    await edit_delete(hinduevent, f"`Completed Process in {ms} seconds`", 3)
