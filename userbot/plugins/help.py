...
async def grpinfo():
    outstr = "**Plugins in HINDUS USERBOT are:**\n\n"
    outstr += f"**👩‍💻 Usage : ** `{cmdprefix}help <plugin name>`\n\n"
    category = ["admin", "fun", "tools", "utils"]
    for hindu in category:
        plugins = GRP_INFO[hindu]
        outstr += f"**{hemojis[hindu]} {hindu.title()} **({len(plugins)})\n"
...
async def cmdlist():
    outstr = "**Total list of Commands in your HINDUS USERBOT are :**\n\n"
    category = ["admin", "fun", "tools", "utils"]
    for hindu in category:
        plugins = GRP_INFO[hindu]
        outstr += f"**{hemojis[hindu]} {hindu.title()} ** - {len(plugins)}\n\n"
...
@hinduub.hindu_cmd(
    ...
    info={
        "header": "To get guide for HINDUS USERBOT.",
...
async def _(event):
    "To get guide for HINDUS USERBOT."
...
async def _(event):
    ...
    if not input_str:
        ...
        outstr += f"**👩‍💻 Usage : ** `{cmdprefix}help -c <command name>`"
    await edit_or_reply(
        event, outstr, aslink=True, linktext="Total Commands of HINDUS USERBOT are :"
    )
...
async def _(event):
    ...
    if found:
        ...
        out = f"I can't find any such command `{cmd}` in HINDUS USERBOT"
