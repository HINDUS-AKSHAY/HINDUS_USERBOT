...
async def hindulst_of_files(path):
    files = []
...
def sortthings(contents, path):
    ...
    for file in contents:
        hindupath = os.path.join(path, file)
        if os.path.isfile(hindupath):
            hindusort.append(file)
    for file in contents:
        hindupath = os.path.join(path, file)
        if os.path.isdir(hindupath):
            hindusort.append(file)
    return hindusort
...
async def upload(path, event, udir_event, hinduflag=None):  # sourcery no-metrics
    hinduflag = hinduflag or False
    ...
    if os.path.isdir(path):
        ...
        for file in Files:
            hindupath = os.path.join(path, file)
            await upload(Path(hindupath), event, udir_event)
...
async def uploadir(event):
    ...
    if os.path.isdir(path):
        ...
        UPLOAD_.uploaded = 0
        await upload(path, event, udir_event, hinduflag=flag)
        end = datetime.now()
