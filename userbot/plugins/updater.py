...
from userbot import HEROKU_APP, UPSTREAM_REPO_URL, hinduub
...
async def upstream(event):
    ...
    if conf == "" and not force_update:
        ...
        await event.delete()
        return await event.respond(f"do `{cmdhd}update deploy` to update the HINDUS USERBOT")
...
async def upstream(event):
    ...
    off_repo = "https://github.com/OFFICIAL-SAMEER/nekopack"
    os.chdir("/app")
    try:
        txt = "`Oops.. Updater cannot continue due to "
        txt += "some problems occured`\n\n**LOGTRACE:**\n"
