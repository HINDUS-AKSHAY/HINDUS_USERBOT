...
async def tmuter(event):  # sourcery no-metrics
    ...
    hinduevent = await edit_or_reply(event, "`muting....`")
    user, reason = await get_user_from_event(event, hinduevent)
    if not user:
        return
    if not reason:
        return await hinduevent.edit("you haven't mentioned time, check `.help tmute`")
    reason = reason.split(" ", 1)
    hmm = len(reason)
    hindutime = reason[0].strip()
    reason = "".join(reason[1:]) if hmm > 1 else None
    ctime = await extract_time(hinduevent, hindutime)
    if not ctime:
        return
    if user.id == event.client.uid:
        return await hinduevent.edit("Sorry, I can't mute myself")
    try:
        await hinduevent.client(
            EditBannedRequest(
                event.chat_id,
                user.id,
...
async def tban(event):  # sourcery no-metrics
    ...
    hinduevent = await edit_or_reply(event, "`banning....`")
    user, reason = await get_user_from_event(event, hinduevent)
    if not user:
        return
    if not reason:
        return await hinduevent.edit("you haven't mentioned time, check `.help tban`")
    reason = reason.split(" ", 1)
    hmm = len(reason)
    hindutime = reason[0].strip()
    reason = "".join(reason[1:]) if hmm > 1 else None
    ...
    if not ctime:
        return
    if user.id == event.client.uid:
        return await hinduevent.edit("Sorry, I can't ban myself")
    await hinduevent.edit("`Whacking the pest!`")
    try:
        await event.client(
            EditBannedRequest(
                event.chat_id,
