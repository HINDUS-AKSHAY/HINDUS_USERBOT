...
from ..funcs import CMD_INFO, PLG_INFO
from ..funcs.data import _sudousers_list, sudo_enabled_cmds
...
@hinduub.hindu_cmd(
    ...
    info={
        "header": "To enable or disable sudo of your HINDUS USERBOT.",
...
async def chat_blacklist(event):
    "To enable or disable sudo of your HINDUS USERBOT."
...
async def _(event):
    ...
    try:
        ...
        sudousers = {}
    if len(sudochats) == 0:
        return await edit_delete(event, "__There are no sudo users for your HINDUS USERBOT.__")
    result = "**The list of sudo users for your HINDUS USERBOT are :**\n\n"
...
async def _(event):  # sourcery no-metrics
    ...
    if input_str[0] == "-all":
        ...
        for plugin in input_str:
            if plugin not in PLG_INFO:
                errors += f"`{plugin}` __There is no such plugin in your HINDUS USERBOT__.\n"
            else:
        ...
        for cmd in input_str:
            if cmd not in CMD_INFO:
                errors += f"`{cmd}` __There is no such command in your HINDUS USERBOT__.\n"
    ...
    result = f"__Successfully enabled __ `{len(loadcmds)}` __ for HINDUS USERBOT sudo.__\n"
    output = (
        result + "**Bot is reloading to apply the changes. Please wait for a minute**\n"
    )
    if errors != "":
        output += "\n**Errors:**\n" + errors
    msg = await edit_or_reply(hinduevent, output)
    await event.client.reload(msg)
...
async def _(event):  # sourcery no-metrics
    ...
    result = f"__Successfully disabled __ `{count}` __ for HINDUS USERBOT sudo.__\n"
    output = (
        result + "**Bot is reloading to apply the changes. Please wait for a minute**\n"
    )
    if errors != "":
...
hinduub.loop.create_task(_init())
