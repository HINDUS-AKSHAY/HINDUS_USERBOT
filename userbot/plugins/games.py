...
async def get_task(mode, choice):
    url = "https://psyhindugames.com/api/tod-v2/"
    data = {
        "id": "truth-or-dare",
        "language": "en",
    ...
    headers = {
        "referer": "https://psyhindugames.com/app/truth-or-dare/?utm_campaign=tod_website&utm_source=tod_en&utm_medium=website"
    }
...
@hinduub.hindu_cmd(
    ...
    info={
        ...
        "note": "U need to give input as 1 for classic , 2 for kids , 3 for party , 4 for hot and 5 for mixed. if you want to give multiple hindugories then use numbers without space like 12",
        "example": [
            "{tr}task",
            "{tr}task 2",
            "{tr}task 123",
...
async def truth_dare_task(event):
    ...
    if taskmode == "task":
        hinduevent = await edit_or_reply(event, "Getting a random task for you.....")
    ...
    try:
        ...
        if taskmode == "truth":
            await hinduevent.edit(f"**The truth task for you is**\n`{task}`")
        else:
            await hinduevent.edit(f"**The dare task for you is**\n`{task}`")
    except Exception as e:
        await edit_delete(hinduevent, f"**Error while getting task**\n`{e}`", 7)
...
async def igame(event):
    ...
    if input_str not in game_code:
        hinduevent = await edit_or_reply(event, "`Give me a correct game code...`")
        await asyncio.sleep(1)
        await edit_delete(
            hinduevent, f"**Available Game Codes & Names :-**\n\n{game_list}", time=60
        )
    else:
        await edit_or_reply(
            event,
