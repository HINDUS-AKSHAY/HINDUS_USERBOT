...
async def _(event):
    ...
    if not replied_user:
        return
    hinduevent = await edit_or_reply(event, "`Fetching userinfo wait....`")
...
""".format(
    ...
    await edit_or_reply(hinduevent, caption)
...
async def who(event):
    ...
    if not replied_user:
        return
    hindu = await edit_or_reply(event, "`Fetching userinfo wait....`")
    replied_user = await event.client(GetFullUserRequest(replied_user.id))
    try:
        photo, caption = await fetch_info(replied_user, event)
    except AttributeError:
        return await edit_or_reply(hindu, "`Could not fetch info of that user.`")
    ...
    try:
        ...
        await hindu.edit(caption, parse_mode="html")
