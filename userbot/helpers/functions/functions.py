...
async def hide_inlinebot(borg, bot_name, text, chat_id, reply_to_id, c_lick=0):
    ...
    hindu = await sticcers[c_lick].click("me", hide_via=True)
    if hindu:
        await borg.send_file(int(chat_id), hindu, reply_to=reply_to_id)
        await hindu.delete()
...
async def waifutxt(text, chat_id, reply_to_id, bot):
    ...
    hindu = await sticcers[0].click("me", hide_via=True)
    if hindu:
        await bot.send_file(int(chat_id), hindu, reply_to=reply_to_id)
        await hindu.delete()
