...
async def admin_groups(hinduub):
    hindugroups = []
    async for dialog in hinduub.iter_dialogs():
...
async def extract_time(hindu, time_val):
    if any(time_val.endswith(unit) for unit in ("s", "m", "h", "d", "w")):
        unit = time_val[-1]
        time_num = time_val[:-1]  # type: str
        if not time_num.isdigit():
            await hindu.edit("Invalid time amount specified.")
            return None
        if unit == "s":
