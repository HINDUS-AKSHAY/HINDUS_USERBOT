...
from userbot import Config, hinduub
...
async def send_flood_alert(user_) -> None:
    ...
    if FloodConfig.ALERT and (user_.id in FloodConfig.ALERT.keys()):
        ...
        try:
            ...
            if BOTLOG:
                await hinduub.tgbot.send_message(
                    BOTLOG_CHATID,
    ...
    if found:
        if flood_count >= FloodConfig.AUTOBAN:
            if user_.id in Config.SUDO_USERS:
                ...
                if BOTLOG:
                    await hinduub.tgbot.send_message(BOTLOG_CHATID, sudo_spam)
            else:
        ...
        if not fa_id:
            return
        try:
            msg_ = await hinduub.tgbot.get_messages(BOTLOG_CHATID, fa_id)
        ...
        if BOTLOG:
            fa_msg = await hinduub.tgbot.send_message(
                BOTLOG_CHATID,
                flood_msg,
                buttons=buttons,
            )
        try:
            chat = await hinduub.tgbot.get_entity(BOTLOG_CHATID)
            await hinduub.tgbot.send_message(
                Config.OWNER_ID,
            ...
            if BOTLOG:
                await hinduub.tgbot.send_message(BOTLOG_CHATID, "**Unblock your bot !**")
...
async def bot_pm_ban_cb(c_q: CallbackQuery):
    ...
    try:
        user = await hinduub.get_entity(user_id)
    except Exception as e:
