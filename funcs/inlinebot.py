...
LIONLOGO = "https://telegra.ph/file/9299f043e1b27a12dd944.jpg"
...
async def inline_handler(event):  # sourcery no-metrics
    ...
    if query_user_id == Config.OWNER_ID or query_user_id in Config.SUDO_USERS:
        ...
        if query.startswith("**HINDUS USERBOT"):
            buttons = [
                (
            ...
            _result = main_menu()
            result = builder.article(
                title="© HINDUS USERBOT Help",
                description="Help menu for HINDUS USERBOT",
                text=_result[0],
                buttons=_result[1],
                link_preview=False,
            )
        ...
        buttons = [
            (
                Button.url("Source code", "https://github.com/OFFICIAL-SAMEER/HINDUS USERBOT"),
                Button.url(
                    "Deploy",
        ...
        result = types.InputBotInlineResult(
            ...
            url="https://github.com/OFFICIAL-SAMEER/HINDUS USERBOT",
            thumb=photo,
            content=photo,
            send_message=types.InputBotInlineMessageMediaAuto(
                reply_markup=markup, message=text, entities=msg_entities
...
@hinduub.tgbot.on(CallbackQuery(data=re.compile(b"close")))
@check_owner
...
@hinduub.tgbot.on(CallbackQuery(data=re.compile(b"check")))
...
@hinduub.tgbot.on(CallbackQuery(data=re.compile(b"(.*)_menu")))
@check_owner
async def on_plug_in_callback_query_handler(event):
...
@hinduub.tgbot.on(CallbackQuery(data=re.compile(rb"mainmenu")))
@check_owner
async def on_plug_in_callback_query_handler(event):
    _result = main_menu()
